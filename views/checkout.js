...
(function (root) {
  ...
  /* Редакция политики, под которой покупатель ставит галочку.
     Меняешь текст politika.html — поднимай и эту дату. */
  var PDN_DOC = '26.08.2026';

  /* Согласие в человеческом виде: «да · 26.08.2026 14:03 · политика ред. …».
     Дата — момент галочки, не момент отправки заказа. */
  function consentLine(c) {
    if (!c || !c.at) return 'Согласие на обработку ПД: не отмечено';
    var d = new Date(c.at);
    var two = function (n) { return (n < 10 ? '0' : '') + n; };
    var kogda = isNaN(d.getTime()) ? String(c.at)
      : two(d.getDate()) + '.' + two(d.getMonth() + 1) + '.' + d.getFullYear() +
        ' ' + two(d.getHours()) + ':' + two(d.getMinutes());
    return 'Согласие на обработку ПД: да · ' + kogda +
      ' · политика ред. ' + (c.doc || PDN_DOC);
  }

  /* Текст заказа для буфера и Telegram.
   * o = { num, items:[{t,qty,n}], name, contact,
   *       delivery:{way,place}, pay, comment?, consent?:{at,doc} } */
  function orderText(o) {
    ...
    lines.push(consentLine(o.consent));
  ...
  function render(el0) {
    ...
    function fillStep(n, body, d) {
      if (n === 1) {
        ...
        cb.addEventListener('change', function () {
          ...
          /* доказательство согласия: когда отметили и под какой редакцией */
          TMB.store.consent.set('pdnAt', cb.checked ? new Date().toISOString() : '');
          TMB.store.consent.set('pdnDoc', cb.checked ? PDN_DOC : '');
        });
    ...
    function submit() {
      ...
      var order = {
        ...
        consent: {
          at: TMB.store.consent.get('pdnAt') || '',
          doc: TMB.store.consent.get('pdnDoc') || PDN_DOC,
        },
      };
